...
const Restaurant = () => {
  ...
  useEffect(() => {
    const fetchRestaurantDetails = async (idStr: string) => {
      ...
      try {
        const response = await api.get(`/api/catalog/${idStr}`);
        ...
        {restaurant && (
            <>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-10 mt-10'>
                <div className='flex items-center justify-center'>
                  <img
                      src={finalImage}
                      alt={restaurant.name}
                      className='rounded-3xl w-full h-[450px] object-cover shadow-2xl shadow-black/20'
                  />
                </div>
              ...
              <div className='mt-20 max-w-4xl mx-auto'>
                <h2 className='text-3xl font-black mb-10 border-l-8 border-orange-500 pl-4 uppercase'>
                  Iskustva posetilaca
                </h2>
                ...
                <div className='bg-white p-8 rounded-3xl shadow-lg border border-gray-100 mb-12'>
                  ...
                  <div className='flex gap-2 mb-4'>
                    {[1, 2, 3, 4, 5].map((num) => (
                        <button
                            key={num}
                            onClick={() => setOcena(num)}
                            className={`text-2xl transition-all ${
                                ocena >= num ? 'scale-110' : 'grayscale opacity-30'
                            }`}
                        >
                          ⭐
                        </button>
                ...
                <div className='space-y-6'>
                  {restaurant.recenzije && restaurant.recenzije.length > 0 ? (
                      restaurant.recenzije.map((r: any) => (
                          <div
                              key={r.id}
                              className='bg-gray-50 p-6 rounded-2xl border-l-4 border-orange-500 flex flex-col gap-2 shadow-sm'
                          >
                            <div className='flex justify-between items-center'>
                              <p className='font-black text-lg text-gray-800 uppercase flex items-center gap-2'>
                                {r.user?.name || r.user?.ime || 'Korisnik'}
                                {r.user?.role === 'admin' && (
                                    <span className='text-[9px] bg-red-500 text-white px-2 py-0.5 rounded'>
                            ADMIN
                          </span>
                                )}
                              </p>

                              <span className='bg-white px-3 py-1 rounded-full text-orange-600 font-bold border border-orange-100'>
                        ⭐ {r.ocena}
                      </span>
                            </div>
                      ...
                      <div className='text-center py-10 bg-gray-50 rounded-3xl border-2 border-dashed border-gray-200'>
                        <p className='text-gray-400'>
                          Nema komentara. Budite prvi koji će oceniti ovaj restoran!
                        </p>
                      </div>
                  )}
                </div>
